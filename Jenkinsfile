node {
    withCredentials([[$class: 'UsernamePasswordMultiBinding',
        ...
        stage('Pull') {
            try {
                git branch: 'main', url: 'https://github.com/DS0610/fisa05-jenkins-practice.git'
        ...
        stage('Deploy') {
            sshagent(credentials: ['ec2-fastapi-server']) {
                ...
                if (env.BUILD_NUMBER.toInteger() != 1) {
                    sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@43.203.127.249 "sudo docker rm -f docker-fastapi"')
                } else {
